...
export default class AppsterModel {
    ...
    getCurrentWork() {
        return this.currentWork = this.recentWork[0];
    }
    ...
    createNewWork(newName) {
        return new AppWork(newName);
    }

    checkValidity = (newName) => {
        if(newName.length < 1)
        {
            this.view.showChar();
            return;
        ...
        for (let i = 0; i < this.recentWork.length; i++)
        {
            if(newName == this.recentWork[i].getName())
            {
